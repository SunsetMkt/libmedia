...
import { AVMediaType, AVCodecID, type AVStream, AVStreamMetadataKey } from '@libmedia/avutil'
    ...
    if (stream.codecpar.codecType === AVMediaType.AVMEDIA_TYPE_VIDEO) {
      ...
      while (ioReader.getPos() < endPos) {
        ...
        if (type === mktag(BoxType.AVCC)) {
          ...
          else if (isobmffContext.parseOneBox && ContainerBoxs.some((boxType) => {
            return mktag(boxType) === type
          })) {
            await isobmffContext.parseOneBox(
              ioReader,
              stream,
              {
                type,
      ...
      if (!isobmffContext.isom || (version === 0 && subVersion > 0)) {
        if (subVersion === 1) {
